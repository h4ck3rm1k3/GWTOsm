...
public class GeneratedStyleEvaluator extends StyleEvaluator {
	protected Style mturning_circle = new Style("Style") {
		...
		protected Rule mRules0 = new Rule("Rule") {
			// DEBUG adding Rules
			protected PointSymbolizer mSymbol1 = new PointSymbolizer(
					"symbols/turning_circle.15.png", "png", "15", "15");
		...
		protected Rule mRules1 = new Rule("Rule") {
			// DEBUG adding Rules
			protected PointSymbolizer mSymbol2 = new PointSymbolizer(
					"symbols/turning_circle.19.png", "png", "19", "19");
	...
	protected Style mhighway_area_casing = new Style("Style") {
		...
		protected Rule mRules2 = new Rule("Rule") {
			...
			protected Filter mFilter1 = new Filter() {
				public void exec() {
					Filter(Concat(Or(Concat(
							Or(Concat(And(Concat(Not(Parens(Condition(getObj()
									.gethighway().equals_(CONST_unclassified)))))))),
							And(Concat(Not(Parens(Condition(getObj()
									.gethighway().equals_(CONST_residential))))))))));
				};// end of exec
			}; // end of Filter
			...
			protected stroke_width mCSS1 = new stroke_width("stroke-width") {
				// DEBUG adding CSS
				protected stroke_width mCSSConst = new stroke_width(
						"stroke-width", "1");
				...
				public void exec() {
					Filter(Concat(Or(Concat(
							Or(Concat(
									...
									And(Concat(Not(Parens(Condition(getObj()
											.gethighway()
											.equals_(CONST_service)))))))),
							And(Concat(Not(Parens(Condition(getObj()
									.gethighway().equals_(CONST_pedestrian))))))))));
				};// end of exec
			}; // end of Filter
		...
		protected Rule mRules4 = new Rule("Rule") {
			...
			protected Filter mFilter3 = new Filter() {
				public void exec() {
					Filter(Concat(Or(Concat(And(Concat(Not(Parens(Condition(getObj()
							.gethighway().equals_(CONST_track))))))))));
				};// end of exec
			}; // end of Filter
		...
		class Check_highway extends typebase {
			public highway obj;
			protected Filter mcode0 = new Filter() {
				public void exec() {
					Filter(Concat(Or(Concat(
							Or(Concat(And(Concat(Not(Parens(Condition(getObj()
									.gethighway().equals_(CONST_unclassified)))))))),
			...
			protected Filter mcode1 = new Filter() {
				public void exec() {
					Filter(Concat(Or(Concat(
							Or(Concat(
									Or(Concat(
			...
			protected Filter mcode2 = new Filter() {
				public void exec() {
					Filter(Concat(Or(Concat(And(Concat(Not(Parens(Condition(getObj()
							.gethighway().equals_(CONST_track))))))))));
				}
	...
	protected Layer mLayer0 = new Layer("Layer") {
		...
		protected Parameter mtable2 = new Parameter(
				"_point where highway='turning_circle') as turning_circle");
	...
	protected Layer mLayer1 = new Layer("Layer") {
		...
		protected Parameter mtable5 = new Parameter(
	...
	class Check_highway extends typebase {
		public highway obj;
		protected Filter mcode3 = new Filter() {
			public void exec() {
				Filter(Concat(Or(Concat(
						Or(Concat(And(Concat(Not(Parens(Condition(getObj()
								.gethighway().equals_(CONST_unclassified)))))))),
						And(Concat(Not(Parens(Condition(getObj().gethighway()
								.equals_(CONST_residential))))))))));
			}
		};
		protected Filter mcode4 = new Filter() {
			public void exec() {
				Filter(Concat(Or(Concat(Or(Concat(Or(Concat(
						Or(Concat(And(Concat(Not(Parens(Condition(getObj()
								.gethighway().equals_(CONST_path)))))))),
						And(Concat(Not(Parens(Condition(getObj().gethighway()
								.equals_(CONST_footway)))))))),
						And(Concat(Not(Parens(Condition(getObj().gethighway()
								.equals_(CONST_service)))))))),
						And(Concat(Not(Parens(Condition(getObj().gethighway()
								.equals_(CONST_pedestrian))))))))));
			}
		};
		protected Filter mcode5 = new Filter() {
			public void exec() {
				Filter(Concat(Or(Concat(And(Concat(Not(Parens(Condition(getObj()
						.gethighway().equals_(CONST_track))))))))));
			}
	...
	public static final highway unclassified = null;
};// end StyleEvaluator
