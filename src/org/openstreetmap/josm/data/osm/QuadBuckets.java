...
public class QuadBuckets implements Collection
{
    ...
    class QBLevel
    {
        ...
        boolean remove_content(OsmPrimitive o)
        {
       ...
       QBLevel[] newChildren()
        {
        	...
        	//return T.createArray(QuadTiling.TILES_PER_LEVEL);
        	//return T.(QuadTiling.TILES_PER_LEVEL);
            //return (QBLevel[])Array.newInstance(this.getClass(), QuadTiling.TILES_PER_LEVEL);
    	   
    	   return new QBLevel[QuadTiling.TILES_PER_LEVEL]; 
        }
        ...
        void __split() {
            ...
            List tmpcontent = content;
            content = null;
            children = newChildren();
            for (Object o: tmpcontent) {
                add((OsmPrimitive) o);
            }
            if (!hasChildren()) {
        ...
        boolean __add_content(OsmPrimitive o)
        {
            boolean ret = false;
        ...
        boolean matches(OsmPrimitive o, BBox search_bbox)
        {
        ...
        private void search_contents(BBox search_bbox, List result)
        {
            ...
            for (Object o : content) {
                if (matches((OsmPrimitive) o, search_bbox)) {
                    result.add(o);
                }
            }
            if (debug) {
    ...
    public boolean add(OsmPrimitive n) {
        synchronized (split_lock) {
            root.add(n);
            size++;
            return true;
    ...
    private OsmPrimitive convert(Object raw)
    {
        return (OsmPrimitive)raw;
    }
    public boolean remove(Object o) {
        return this.remove(convert(o));
    }
    public boolean remove(OsmPrimitive o) {
        synchronized (split_lock) {
    ...
    public  OsmPrimitive[] toArray(OsmPrimitive[] template)
    {
        return (OsmPrimitive[]) this.toArrayList().toArray(template);
    }
    class QuadBucketIterator implements Iterator
    {
        ...
        OsmPrimitive peek()
        {
            ...
            return (OsmPrimitive) current_node.content.get(content_index);
        }
        public OsmPrimitive next()
        {
            OsmPrimitive ret = peek();
            content_index++;
            if (debug) {
    ...
    public List search(BBox search_bbox) {
        if (debug) {
