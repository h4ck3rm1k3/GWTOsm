...
public class AutoScaleAction extends JosmAction {
    ...
    public static void zoomToSelection() {
        ...
        Collection<OsmPrimitive> sel = getEditLayer().data.getSelected();
        if (sel.isEmpty()) {
...
//            );
        ...
        if (bboxCalculator.getBounds() != null) {
            map.mapView.recalculateCenterScale(bboxCalculator);
        }
    }
    ...
    public void autoScale()  {
        if (map != null) {
            if (mode.equals("previous")) {
                map.mapView.zoomPrevious();
            } else if (mode.equals("next")) {
                map.mapView.zoomNext();
            } else {
                BoundingXYVisitor bbox = getBoundingBox();
                if (bbox != null && bbox.getBounds() != null) {
                    map.mapView.recalculateCenterScale(bbox);
                }
            }
        }
    ...
    private BoundingXYVisitor getBoundingBox() {
        ...
        if (mode.equals("data")) {
            for (Layer l : map.mapView.getAllLayers()) {
            ...
            if (mode.equals("selection")) {
                ...
                if (map.conflictDialog.getConflicts() != null) {
                    sel = map.conflictDialog.getConflicts().getMyConflictParties();
                }
            }
            if (sel.isEmpty()) {
//                JOptionPane.showMessageDialog(
...
//            }
    ...
    protected void updateEnabledState() {
        if ("selection".equals(mode)) {
			...
			if (map == null || mapView == null || map.mapView.getAllLayersAsList().isEmpty()) {
                setEnabled(false);
            } else {
            ...
            setEnabled(map != null && map.mapView != null && map.mapView.hasZoomUndoEntries());
        } else if ("next".equals(mode)) {
            setEnabled(map != null && map.mapView != null && map.mapView.hasZoomRedoEntries());
        } else {
            setEnabled(
                    //Main.isDisplayingMapView()
                    map.mapView.hasLayers()
            );
        }
    }
